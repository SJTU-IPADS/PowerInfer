...
struct llama_model_loader {
    ...
    struct ggml_tensor * create_tensor(struct ggml_context * ctx, const std::pair<std::string, llm_tensor> & tn, const std::vector<int64_t> & ne, ggml_backend_type backend, bool required = true) {
        return create_tensor(ctx, tn.first, ne, backend, required);
    }
        ...
        switch (model.arch) {
            ...
            case LLM_ARCH_QWEN2MOE:
            {
                ...
                for (int i = 0; i < n_layer; ++i) {
                    ...
                    // MoE branch
                    for (uint32_t x = 0; x < hparams.n_expert; ++x) {
                        layer.ffn_gate_exp[x] = ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_GATE_EXP, "weight", i, x), {n_embd, n_ff_exp}, backend_split);
