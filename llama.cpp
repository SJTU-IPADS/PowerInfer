...
tensor_offloading_levels get_offloading_level(llm_tensor tensor) {
    switch (tensor) {
        ...
        case LLM_TENSOR_FFN_GATE_INP: case LLM_TENSOR_FFN_NORM:
...
struct LLM_TN {
    ...
    std::pair<std::string, llm_tensor> operator()(llm_tensor tensor, const std::string & suffix, int bid, int xid) const {
        return std::make_pair(::format(LLM_TENSOR_NAMES[arch].at(tensor).c_str(), bid, xid) + "." + suffix, tensor);
    }
};
            ...
            case LLM_ARCH_BAMBOO:
                {
                    ...
                    for (uint32_t i = 0; i < n_layer; ++i) {
                        ...
                        if (hparams.n_expert > 0) {
                            layer.ffn_gate_inp = ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_GATE_INP, "weight", i), {n_embd}, backend);
                            for (uint32_t x = 0; x < hparams.n_expert; ++x) {
                                ...
                                if (backend == GGML_BACKEND_GPU) {
                                    vram_weights += ggml_nbytes(layer.ffn_up_exp[x]) + ggml_nbytes(layer.ffn_gate_exp[x]) + ggml_nbytes(layer.ffn_down_exp[x]);
                                }
                            }
                        } else {
                            ...
                            if (backend == GGML_BACKEND_GPU) {
                                vram_weights += ggml_nbytes(layer.ffn_gate)  + ggml_nbytes(layer.ffn_down) + ggml_nbytes(layer.ffn_up);
                            }
                        }
                    }
                } break;
