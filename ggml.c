...
static void ggml_compute_forward_mul_mat_axpy_q8_0(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
              struct ggml_tensor * dst) {
    ...
    for (int col_idx = 0; col_idx < nr1; col_idx++) {
        ...
        for (int64_t ir1 = ir10; ir1 < ir10 + dr; ir1++)
        {
            ...
            ggml_axpy_q8_0_q8_0(ne00, src0_row + nb01 * ir1, vy, vy, b, d);
        }
        //     if (ir0 + dr >= nr)
        //         break;
        // }
...
static void ggml_compute_forward_mul_mat_axpy_q4_1(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
              struct ggml_tensor * dst) {
    ...
    // ggml_vec_dot_t    const vec_dot               = type_traits[type].vec_dot;
    enum ggml_type    vec_dot_type          = type_traits[type].vec_dot_type;
    //TODO: not hardcode
    vec_dot_type = GGML_TYPE_Q8_0;
    ggml_from_float_t const from_float_to_vec_dot = type_traits[vec_dot_type].from_float;
    ...
    for (int col_idx = 0; col_idx < nr1; col_idx++) {
        ...
        for (int64_t ir1 = ir10; ir1 < ir10 + dr; ir1++)
        {
            ...
            if (b == 0)
                continue;
            ggml_fp16_t d = nerual[bid].d;
            ggml_axpy_q4_1_q8_1(ne00, src0_row + nb01 * ir1, vy, vy, b, d);
        }
        //     if (ir0 + dr >= nr)
        //         break;
        // }
...
static void ggml_compute_forward_mul_mat_axpy_q5_1(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
              struct ggml_tensor * dst) {
    ...
    for (int col_idx = 0; col_idx < nr1; col_idx++) {
        ...
        for (int64_t ir1 = ir10; ir1 < ir10 + dr; ir1++)
        {
            ...
            if (b == 0)
                continue;
            ggml_fp16_t d = nerual[bid].d;
            ggml_axpy_q5_1_q8_1(ne00, src0_row + nb01 * ir1, vy, vy, b, d);
        }
        //     if (ir0 + dr >= nr)
        //         break;
        // }
...
static void ggml_compute_forward_mul_mat_axpy_q5_0(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
              struct ggml_tensor * dst) {
    ...
    for (int col_idx = 0; col_idx < nr1; col_idx++) {
        ...
        for (int64_t ir1 = ir10; ir1 < ir10 + dr; ir1++)
        {
            ...
            if (b == 0)
                continue;
            ggml_fp16_t d = nerual[bid].d;
            ggml_axpy_q5_0_q8_0(ne00, src0_row + nb01 * ir1, vy, vy, b, d);
        }
        float *res = (float *)((char *)(dst->data) + col_idx * nb1);
        float *tmp = (float *)vy;
        int i;
        ...
        case GGML_OP_AXPY:
            {
                ...
                if (tensor->src[2] == NULL) {
                    ...
                    case GGML_TYPE_Q4_0:
                        ...
                        break;
                    case GGML_TYPE_Q4_1:
                        ggml_compute_forward_mul_mat_axpy_q4_1(params, tensor->src[0], tensor->src[1], tensor);
                        break;
                    case GGML_TYPE_Q5_0:
                        ggml_compute_forward_mul_mat_axpy_q5_0(params, tensor->src[0], tensor->src[1], tensor);
                        break;
                    case GGML_TYPE_Q5_1:
                        ggml_compute_forward_mul_mat_axpy_q5_1(params, tensor->src[0], tensor->src[1], tensor);
                        break;
                    case GGML_TYPE_Q8_0:
                        ggml_compute_forward_mul_mat_axpy_q8_0(params, tensor->src[0], tensor->src[1], tensor);
                        break;
                    case GGML_TYPE_F16:
                        ggml_compute_forward_mul_mat_axpy(params, tensor->src[0], tensor->src[1], tensor);
                        break;
                    default:
                        printf("We do not support type %d\n", tensor->src[0]->type);
                        break;
                    }
